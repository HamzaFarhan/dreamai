...
if __name__ == "__main__":
    lance_model = get_lance_ems_model(name=EMS_MODEL, device=DEVICE)

    # Load and process the Burr documentation
    burr_docs_path = Path(BURR_DOCS_DIR) / "burr_docs.pdf"
    md_result = pdf_to_md_docs(file_path=burr_docs_path)

    # burr_docs = [{**chunk, "file_name": "burr_docs.pdf"} for chunk in md_result.chunks]

    lance_db = lancedb.connect(LANCE_URI)

    # Add documents to LanceDB
    burr_table = add_to_lance_table(
        db=lance_db,
        table_name="burr_docs",
        data=md_result.chunks,
        ems_model=lance_model,
    )
