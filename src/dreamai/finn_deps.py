...
async def list_results_files(ctx: RunContext[FinnDeps]) -> str:
    """
...
def resolve_file_path(
    ctx: RunContext[FinnDeps], file_path: str | Path, suffixes: list[str] = [".xlsx", ".parquet", ".csv"]
) -> Path:
    ...
    for suffix in suffixes:
        paths_to_try += [
            ...
            ctx.deps.dirs.analysis_dir / file_path.name,
            (ctx.deps.dirs.analysis_dir / file_path.stem).with_suffix(suffix),
...
def load_df(ctx: RunContext[FinnDeps], data: pl.DataFrame | str | Path) -> pl.DataFrame:
    if isinstance(data, pl.DataFrame):
        return data
    df_path = resolve_file_path(ctx, data)
...
def load_file(ctx: RunContext[FinnDeps], file_path: str) -> pl.DataFrame:
    df_path = resolve_file_path(ctx, file_path)
